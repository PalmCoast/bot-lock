...
function record(state, action, payload) {
    if (!state.identity)
        return;
...
export function createBotLockServer(home = defaultHome()) {
    ...
    server.tool("botlock_identity_create", "Create or rotate the local Ed25519 agent identity used to sign the audit chain. Rotation keeps the old key out of the new chain.", { label: z.string().optional(), rotate: z.boolean().optional() }, async ({ label, rotate }) => {
        const state = loadState(home);
    ...
    server.tool("botlock_identity_show", "Show the public agent identity (never the private key).", {}, async () => {
        const state = loadState(home);
        if (!state.identity)
            return text({ error: "No identity. Call botlock_identity_create." });
        return text(publicView(state.identity));
    });
    server.tool("botlock_vault_put", "Seal a secret into the AES-GCM vault. The plaintext is not written to the audit log.", {
        name: z.string(),
        value: z.string(),
    }, async ({ name, value }) => {
        const state = loadState(home);
        if (!state.masterKey)
            return text({ error: "Vault master key missing." });
        if (!state.identity)
            return text({ error: "Create an identity first." });
        const sealed = sealSecret(state.masterKey, name, value, state.identity.id);
        if (state.secrets[name])
            sealed.createdAt = state.secrets[name].createdAt;
    ...
    server.tool("botlock_vault_get", "Open a named secret. Use only when the current task needs it. The value is returned to the caller and is not appended to the audit payload.", { name: z.string() }, async ({ name }) => {
        const state = loadState(home);
        if (!state.masterKey)
            return text({ error: "Vault master key missing." });
        const sealed = state.secrets[name];
        if (!sealed)
            return text({ error: `No secret named "${name}".` });
        const value = openSecret(state.masterKey, sealed);
        record(state, "vault.get", { name });
        saveState(state, home);
        return text({ name, value });
    });
    server.tool("botlock_vault_list", "List vault secret names and metadata. Values are not returned.", {}, async () => {
        const state = loadState(home);
        return text(Object.values(state.secrets).map(publicMeta));
    });
    server.tool("botlock_audit_append", "Append a signed, hash-chained audit event. Put only non-secret metadata in payload.", {
        action: z.string(),
    ...
    server.tool("botlock_audit_tail", "Return the latest N audit entries (hashes and actions, not secret values).", { limit: z.number().int().min(1).max(200).optional() }, async ({ limit }) => {
        const state = loadState(home);
        const n = limit ?? 20;
        return text(state.audit.slice(-n));
    });
    server.tool("botlock_audit_verify", "Verify the hash chain and Ed25519 signatures on the local audit log.", {}, async () => {
        const state = loadState(home);
        if (!state.identity)
            return text({ valid: state.audit.length === 0, reason: "No identity." });
        return text(verifyChain(state.audit, state.identity.publicKeyPem));
    });
    server.tool("botlock_scope_check", "Ask Bot Lock whether a tool call is allowed, denied, or needs a human confirmation under the loaded policy.", {
        tool: z.string(),
    ...
    server.tool("botlock_policy_show", "Show the active policy document.", {}, async () => {
        const state = loadState(home);
        return text(state.policy);
    });
    server.tool("botlock_policy_load", "Replace the active policy with a JSON policy document (version 1).", { policy: z.record(z.unknown()) }, async ({ policy }) => {
        const state = loadState(home);
        const next = policy;
    ...
    server.tool("botlock_policy_reset", "Reset policy to the built-in deny-by-default template.", {}, async () => {
        const state = loadState(home);
    ...
    server.tool("botlock_kill_switch", "Engage or release the kill switch. When engaged, every scope check is denied.", {
        engage: z.boolean(),
