...
export function evaluateStatus(state) {
    ...
    if (!identityReady)
        findings.push("No agent identity. Create one before signing audit entries.");
    ...
    if (!state.identity) {
        ...
        if (state.audit.length > 0)
            findings.push("Audit entries exist without a signing identity.");
    }
    else {
        const check = verifyChain(state.audit, state.identity.publicKeyPem);
        auditValid = check.valid;
        if (!check.valid)
            findings.push(`Audit chain invalid at seq ${check.brokenAt}: ${check.reason}`);
    }
