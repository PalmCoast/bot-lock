...
export function checkScope(policy, request) {
    ...
    if (policy.io.inboundScan && request.inputText && hitsDenyPattern(policy.io.denyPatterns, request.inputText)) {
        return confirmOrDeny(policy, tool, "Inbound text matched an I/O deny-pattern (possible injection or secret leak).", "io.inboundScan");
    }
    ...
    if (request.destination) {
        ...
        if (policy.exfil.allowDestinations.length > 0 &&
...
function fromDefault(action, tool) {
    if (action === "allow") {
...
function confirmOrDeny(policy, tool, reason, matchedRule) {
    const rule = policy.tools.allow.find((r) => matchName(r.name, tool));
    if (rule) {
...
function hitsDenyPattern(patterns, text) {
...
function looksLikePrivilegeEscalation(tool, input) {
    const blob = `${tool} ${input ?? ""}`.toLowerCase();
    return (blob.includes("add tool") ||
...
export function engageKillSwitch(policy, reason) {
    return {
        ...policy,
        killSwitch: {
            engaged: true,
...
export function releaseKillSwitch(policy) {
    return {
        ...policy,
        killSwitch: { engaged: false },
    };
